...
//замена телефонов ссылками
  ...
  $(document).on('touchend','header .dropdown-menu .mobile-block > ul > li.title', function(){
    if ($(window).width() < 830) {
      if ($(this).hasClass('opened')) {
        $(this).removeClass('opened');
  ...
  $(document).on('touchend','header .dropdown-menu .mobile-block > ul > li.title + ul li.title-2', function(){
    if ($(window).width() < 830) {
      if ($(this).hasClass('opened')) {
        $(this).removeClass('opened');
        $(this).next('ul').slideUp('slow');
  ...
  $('#popupCallback').css('display', 'none');

  $(document).on('click', '.ajax-mfp', function() {
    var a = $(this);
    $.magnificPopup.open({
      //items: { src: a.attr('data-href') },
      items: { src: '#popupCallback' },
      type: 'inline',
      overflowY: 'scroll',
      //removalDelay: 300,
      ...
      callbacks: {
        open: function() {
          $('#popupCallback').css('display', 'block');
        }
      }
    });
    return false;
